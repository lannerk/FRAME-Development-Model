...
🔴 **FRAME itself is independent of any specific project** (the Requester ruled on 2026-09-26: "frame should
also be independent of any specific project"). The criterion is one testable sentence: **both templates can be
taken to a completely unrelated project without changing a single character.** So: **project-specific paths,
prefixes, machines and subnets go into `ops/verify/paths.env`** (class seed, never synced), and a guard with
nothing configured **skips instead of going red**; the story behind a rule **stays** (it explains the "why") but
is written generically ("someone used `git add -A` and swept 10 files belonging to other seats into their own
commit" rather than a commit id). The guard `ops/verify/check-debrand.sh` scans both templates and the source
repository against the host-side private-word list.
...
| `advisor/` | **The kickoff advisor library** (FRAME itself, class follow): `README` · `principles` · `questions` · `chains` · `stack` · `domains/*`. Every seat **looks it up as needed** when the Requester starts a project / raises a large requirement / asks "how should this be done" (never read whole, **not in the opening read**); the procedure is `ai/rules/requester.md` §4d. 🔴 **Only judgements that hold on another project belong here** — no brand names, product names, people's names or private sources (`check-advisor.sh` scans); what is specific to a project lives in `product/requirements/requester-profile.md` (seed, never flows back) | **Maintainer seat** (the generic library is FRAME itself) | every seat, on demand |
...
| `ai/advisor/*.md` · `ai/advisor/domains/*.md` | no cap | The advisor library. **Not in the opening read, and never read whole** (pick a question from `questions.md`, read one card in `stack.md`), so its length costs no seat anything at session start; **squeezing the line count only squeezes out the "why", and the "why" is the reason it exists**. Guard `check-advisor.sh` tests "every card complete, every question with Why ask and Recommend" |
| `product/requirements/requester-profile.md` | no cap | The Requester's profile (seed, private to the project, **never flows back**). One line appended per ruling, and **old lines are never deleted** (a changed view is written "changed on <date> to …") — delete them and "when was this preference settled" has no answer |
...
| `ai/rules/layout.md` | **265 lines** | Over the limit, move one of its sections into `conventions.md`. **It is not in the opening read**, and the budget table plus the **file-ownership table** (§10) already make up most of it. **Relaxed from 210 to 240 on 2026-09-15**: §10, the ownership table (22 lines), is the **single source** for `commit-round.sh --seat` and for deciding whose red a red is — **moving it out would make the script keep its own copy**, exactly the trap the budget table fell into once. **240 → 255 on 2026-09-22**: landing the R&D line adds a row to each of this file's **three single-source tables** (§2 · §3 · §7). **255 → 265 on 2026-09-26**: the criterion "🔴 FRAME itself is independent of any specific project" was added (the Requester's ruling) plus four rows for the advisor library and the profile (directory duty, budget, ownership) -- **that criterion has to stay in this file**: it is the single source of "what may not enter FRAME", and moving it elsewhere would let the directory, budget and ownership tables each tell their own version |
| `ai/bugs/README.md` | 150 lines | The full procedure for a bug; the index is in `index.md`. **120 → 150**: the "⏸ on hold" section was added 2026-09-24, each of its three boundaries carrying its own "why" -- **squeeze the "why" out and the next person turns a hold into a blanket exemption**, which is the very thing this test guards against |
...
| `ai/advisor/*` | Maintainer seat (FRAME itself; **any seat may propose content, the change lands through the Maintainer and is de-branded first**) |
| `product/requirements/requester-profile.md` | Supervisor seat (it appends the preferences he rules on; **all four seats and the Researcher read it**) |
| `ai/RandD/*` | Researcher seat |
| `ai/mail/to-researcher/*` | **shared** (Researcher, Supervisor and Reviewer) |
...
**Measured twice**: with `git add -A`, one commit swept 10 files the Reviewer and Supervisor seats were editing at that
moment into the Maintainer's commit message; half an hour later another seat's commit nearly swept 6 files the first had
