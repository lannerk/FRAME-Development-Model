...
| `ops/verify/check-debrand.sh` | 🔴 **FRAME itself is independent of any specific project** (the Requester's ruling of 2026-09-26): it scans **both templates and the source repository** against a **host-side private-word list** (`ops/verify/.private-words`, **class skip, never synced**), and a hit is red — product names, directory names, file-name prefixes, private IPs, commit ids and session paths all count. The criterion in one line: **it can be taken to a completely unrelated project without changing a single character.** Two exemptions, each with its reason (the list itself · the provenance in `FRAME-VERSION` / `.baseline`). **No list means SKIP** | Maintainer when changing a template; before syncing the source repository |
| `ops/verify/check-advisor.sh` | The **kickoff advisor library** (`ai/advisor/`) has not decayed into a pile of assertions: every choice card has **Recommendation · Why · Does not apply / Reversible** · every question has **Why ask** and **Recommend** · the profile **is class seed** in `classes.txt` (marking it follow would push his preferences into the open-source repo) · the profile is not inside the library · **no private word leaked into anything that gets synced out** (list `ops/verify/.private-words`, itself class skip). **Skips itself when there is no library** | Maintainer when changing the library; every wrap-up |
