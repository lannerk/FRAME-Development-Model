...
| `ai/mail/to-researcher/from-{supervisor,reviewer}.md` · `ai/mail/to-{supervisor,reviewer}/from-researcher.md` | same paths | **empty mailboxes** (the Researcher exchanges letters with the Supervisor and the Reviewer — the Reviewer pair was opened 2026-09-25; the gate reds any other pairing) |
...
| `ai/advisor/` (the whole tree) | `ai/template/ai/advisor/` | **as-is** (the kickoff advisor library is FRAME itself: generic principles · the question bank · chains · choice cards · domain practice; 🔴 **no brand names, product names, people's names or private sources inside**, `check-advisor.sh` scans for them) |
| `product/requirements/requester-profile.md` | same path | **an empty skeleton** (the Requester's profile: 12 empty sections plus how it grows). 🔴 **Class seed, never flows back** — the content is private to the project |
| `ops/verify/check-advisor.sh` | same path | **as-is** (tests: cards complete · every question with Why ask and Recommend · the profile is seed · no private word leaked; **skips itself when there is no library**) |
| `ops/verify/.private-words` | **not in the template** | the host-side private-word list, **class skip, never synced** — listing the words you hide in a public repo points them out |
