...
# 有项目照着它真长出了四个（提交记录在那个项目自己的维护记录里）。**两件事合起来说明：根目录从来没有守门。**
#
# 【判三条】
...
# 实测过一次：草稿区多出一个 `claude-outputs/<项目>-V2/`（十几份编号章节 + README）——**东西没问题，位置不对**：
...
for d in claude-outputs/*/; do
  ...
  for e in "$d"*; do
    ...
    # 实测：审查/开发那边建了 `claude-outputs/<项目>-V2/`（00-总纲、10-规划…12 份编号章节 + README.md），
